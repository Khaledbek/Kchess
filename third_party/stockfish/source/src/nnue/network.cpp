...
#if defined(_WIN32)
        throw std::runtime_error("Stockfish NNUE network could not be loaded: " + evalfilePath);
#else
        exit(EXIT_FAILURE);
#endif
    }
...
void Network<Arch, Transformer>::load_user_net(const std::string& dir,
    ...
    // KChess passes UTF-8 paths from its Windows DLL. Constructing the stream
    // through filesystem::path preserves non-ASCII Windows path components.
#if defined(_WIN32)
    const auto combined = dir + evalfilePath;
    const std::u8string utf8Path(
      reinterpret_cast<const char8_t*>(combined.data()), combined.size());
    std::ifstream stream(std::filesystem::path(utf8Path), std::ios::binary);
#else
