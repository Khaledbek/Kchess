...
- `"provider": "auto"` (the shipped default) selects the first id in `"autoOrder"` whose key file exists, once at Coach startup. Gemini leads the order so the free tier stays preferred while its key exists; a failed call never falls back to another provider.
