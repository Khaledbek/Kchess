// -----------------------------------------------------------------------------
// Section: Opening node rendering from native board and progress DTOs
// -----------------------------------------------------------------------------
...
class SkillTreeNodeWidget extends StatelessWidget {
              ...
              SizedBox(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Expanded(
                      child: _summary(context, strings, theme, accent, state),
                    ),
                  ],
                ),
              ),
  ...
  Widget _preview(BuildContext context) {
    final resolved = node.data.position;
  ...
  Widget _summary(
    ...
    return Column(
      ...
      children: [
        Row(
          children: [
            if ((node.data.eco ?? '').isNotEmpty)
              _ecoBadge(theme, node.data.eco!),
            const Spacer(),
  ...
  Widget _masteryDots(ThemeData theme, Color accent, NodeProgressState state) {
    final filled = state == NodeProgressState.mastered
        ? node.data.masteryThreshold
        : progress.successStreak;

    return Row(
      children: [
        for (var i = 0; i < node.data.masteryThreshold; i++)
          Padding(
  ...
  Widget _expandButton(BuildContext context, AppLocalizations strings) {
    ...
    return SizedBox(
      ...
      child: IconButton.filledTonal(
        ...
        visualDensity: VisualDensity.compact,
        tooltip:
            '$label · ${strings.trainingOpeningTreeVariations(node.data.childCount)}',
        ...
        icon: Icon(
          node.expanded ? Icons.unfold_less_rounded : Icons.hub_outlined,
        ),
      ),
    );
  }
