...
int main() {
  try {
    ...
    expect(beginner.verification_time_budget_ms == 150
               && pre_native.verification_time_budget_ms < 400
               && strong.verification_time_budget_ms <= 650,
           "Update 65 keeps Elo-scaled wall-clock safety caps tight on slow hardware");
    expect(maximum.verification_node_budget == 180000
               && maximum.verification_time_budget_ms == 1400,
           "3200 keeps its full-strength node budget while only tightening the emergency time cap");

    const auto native_club = kchess::plan_bot_move(1500, 0.995, 0.25, 20);
    ...
    const auto low_best = kchess::plan_bot_move(1200, 0.0, 0.0, 20);
    const auto low_tail = kchess::plan_bot_move(1200, 0.999, 0.85, 20);
    const auto low_opening = kchess::plan_bot_move(1200, 0.999, 0.85, 2);
    ...
    expect(low_opening.diversity_window_cp > low_tail.diversity_window_cp,
           "opening positions permit a wider near-equal repertoire pool");

    auto equal_plan = kchess::plan_bot_move(1200, 0.0, 0.80, 2);
    equal_plan.target_loss_cp = 0;
    equal_plan.maximum_verified_loss_cp = 100;
    const auto varied = kchess::choose_scout_bot_move(
        std::vector<kchess::EngineLine>{line(1, 0), line(2, -12), line(3, -28), line(4, -95)},
        equal_plan);
    expect(varied.move != "m4",
           "opening diversity never reaches a move outside the near-equal window");
    expect(varied.move == "m2" || varied.move == "m3" || varied.move == "m1",
           "opening diversity is selected only from genuinely close candidates");

    auto strict_plan = kchess::plan_bot_move(1200, 0.0, 0.99, 20);
    strict_plan.target_loss_cp = 0;
    strict_plan.diversity_window_cp = 18;
    strict_plan.maximum_verified_loss_cp = 180;
    const auto strict_choice = kchess::choose_scout_bot_move(
        std::vector<kchess::EngineLine>{line(1, 0), line(2, -10), line(3, -45)},
        strict_plan);
    expect(strict_choice.move != "m3",
           "quiet-position diversity stays inside its tighter equality window");
    ...
    kchess::BotMovePlan diversity_verify_plan;
    diversity_verify_plan.target_loss_cp = 0;
    diversity_verify_plan.diversity_window_cp = 25;
    diversity_verify_plan.maximum_verified_loss_cp = 200;
    const kchess::BotMoveChoice diversity_choice{.move = "m2", .rank = 2, .probability = 1.0};
    const auto diversity_rejected = kchess::finalize_verified_bot_move(
        std::vector<kchess::EngineLine>{line(1, 0), line(2, -70)},
        diversity_choice, diversity_verify_plan);
    expect(diversity_rejected.move == "m1",
           "a diversity-only alternative is rejected if verification shows it was not actually near-equal");

    const auto max_low_roll = kchess::plan_bot_move(3200, 0.0, 0.0, 0);
    const auto max_high_roll = kchess::plan_bot_move(3200, 0.999999, 0.999999, 30);
