...
struct BotMovePlan {
  ...
  int diversity_window_cp{0};
  double diversity_roll{0.0};
};
...
// models human-sized inaccuracies instead of asking for an arbitrary rank. A second
// independent random sample is reserved for weighted choice among near-equivalent
// candidates; opening plies allow slightly more repertoire variety without blunders.
BotMovePlan plan_bot_move(
    int requested_elo,
    double quality_random,
    double diversity_random,
    int position_ply);
