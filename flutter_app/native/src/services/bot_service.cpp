...
int scaled_milliseconds(const int base, const double multiplier) {
  // Tactical positions may deserve substantially more nodes, but letting the
  // hard wall-clock cap grow by the same factor recreates the long stalls this
  // budget system is meant to prevent. A square-root scale keeps tactical
  // headroom while bounding latency on slower devices.
  return std::max(
      1,
      static_cast<int>(std::lround(base * std::sqrt(std::max(0.0, multiplier)))));
}
...
std::string BotService::job_json(const BotJob& job) const {
  nlohmann::json json{
      ...
      {"diversityWindowCp", job.plan.diversity_window_cp},
...
std::string BotService::start_move_json(
  ...
  job->profile = profile;
  // Native UCI_Elo owns its own randomized candidate choice, so consuming KChess
  // RNG values there would be redundant. The low-Elo path uses two independent
  // samples: one for intended CP loss and one for near-equal move diversity.
  const bool kchess_selects_candidates =
      requested_elo != kBotEloMaximum && !profile.use_stockfish_limit_strength;
  job->plan = plan_bot_move(
      requested_elo,
      kchess_selects_candidates ? next_random_unit() : 0.0,
      kchess_selects_candidates ? next_random_unit() : 0.0,
...
void BotService::run_job(const std::shared_ptr<BotJob>& job) noexcept {
  try {
    ...
    if (budget.forced_move.empty() && job->requested_elo == kBotEloMaximum) {
      ...
      // Phase 1: cheap root scout. The same small pool now serves two purposes:
      // matching the pre-sampled CP-loss target and finding near-equivalent moves
      // for weighted variety, without adding another Stockfish search.
