...
- `coach_provider.json` selects the external coach provider (`"provider"`) and holds one settings block per provider under `"providers"`: model, output/timeout bounds and conservative local quota guards. The selected id also names its key file, `secrets/<provider>_api_key.txt`. `"provider": "auto"` selects the first id in `"autoOrder"` (default: gemini, deepseek, claude, openai) whose key file exists; the choice is made once when the Coach starts.
